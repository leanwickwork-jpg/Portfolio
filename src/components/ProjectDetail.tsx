  ...
  return (
    <div className="min-h-screen bg-slate-50 text-neutral-900 transition-colors duration-300 relative pb-24 pt-28">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                    ...
                    {vid.isEmbed || (vid.url.startsWith('http') && (vid.url.includes('drive.google.com') || vid.url.includes('youtube.com') || vid.url.includes('youtu.be') || vid.url.includes('tiktok.com'))) ? (
                      ...
                      <div className="w-full h-full flex flex-col items-center justify-center bg-neutral-900/95 text-neutral-300 text-xs text-center p-5 select-none animate-fade-in">
                        ...
                        <p className="text-[10px] text-neutral-400 leading-relaxed px-1">
                          {lang === 'vi' 
                            ? 'Vui lòng tải tệp này lên thư mục dự án để đồng bộ vĩnh viễn, hoặc sử dụng bộ tải lên ở dưới.' 
                            : 'Please upload this file to the project assets folder, or use the interactive uploader below.'}
                        </p>
                        <button
                          onClick={() => {
                            const elem = document.getElementById('uploader-zone');
                            if (elem) elem.scrollIntoView({ behavior: 'smooth' });
                          }}
                          className="mt-3 px-2.5 py-1.5 bg-neutral-800 hover:bg-neutral-700 text-neutral-200 rounded-lg text-[9px] font-mono border border-neutral-700 transition cursor-pointer"
                        >
                          {lang === 'vi' ? 'Đi tới bộ tải lên ⬇' : 'Go to uploader ⬇'}
                        </button>
                      </div>
                    ) : (
                      <video 
        ...
        {/* Interactive Client-Side Uploader Zone (tôi sẽ tải ảnh hoặc video lên hoặc kèm link) */}
        <div id="uploader-zone" className="bg-white rounded-3xl border border-neutral-200/80 p-6 sm:p-10 shadow-lg text-left relative overflow-hidden mb-12">
          ...
          <div className="flex items-center gap-2 mb-6 border-b border-neutral-100 pb-4">
            ...
            <h3 className="text-base sm:text-lg font-bold text-neutral-950 font-sans tracking-tight">
              {translate.uploadZoneTitle}
            </h3>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
            ...
            {/* Form A: Add Photo */}
            <div className="bg-slate-50/50 rounded-2xl border border-neutral-200/60 p-5 sm:p-6">
              <div className="flex items-center gap-2 mb-4">
                <ImageIcon className="w-5 h-5 text-blue-500" />
                <h4 className="text-xs sm:text-sm font-bold text-neutral-900 uppercase tracking-wide font-mono">
                  {lang === 'vi' ? 'THÊM ẢNH MỚI' : 'ADD NEW PHOTO'}
                </h4>
              </div>

              <form onSubmit={handleAddPhotoByLink} className="space-y-4">
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder={translate.uploadPhotoPlaceholder}
                    className="w-full pl-10 pr-4 py-3 bg-white border border-neutral-220 rounded-xl text-xs font-semibold focus:outline-none focus:border-blue-500 shadow-xs"
                  />
                </div>
                ...
                <input
                  ...
                  placeholder={translate.photoCaption}
                  className="w-full px-4 py-3 bg-white border border-neutral-220 rounded-xl text-xs font-semibold focus:outline-none focus:border-blue-500 shadow-xs"
                />

                {/* Local Upload Simulator */}
                <div className="border border-dashed border-neutral-220/80 rounded-xl p-4 bg-white hover:bg-neutral-50/80 duration-150 transition-colors text-center relative flex flex-col items-center justify-center cursor-pointer">
                  <input 
                    type="file" 
                    accept="image/*" 
                  ...
                  <span className="text-[11px] font-semibold text-neutral-500">{translate.dragDrop}</span>
                </div>

                <button
                  type="submit"
                  disabled={!photoUrlInput.trim()}
                  className="w-full py-3 rounded-xl bg-blue-600 hover:bg-blue-500 active:scale-99 duration-200 text-white font-bold text-xs cursor-pointer shadow-sm disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-1.5"
                >
            ...
            <div className="bg-slate-50/50 rounded-2xl border border-neutral-200/60 p-5 sm:p-6">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h4 className="text-xs sm:text-sm font-bold text-neutral-900 uppercase tracking-wide font-mono">
                  {lang === 'vi' ? 'THÊM THƯỚC PHIM / VIDEO' : 'ADD NEW VIDEO RECORD'}
                </h4>
              </div>
              ...
              <form onSubmit={handleAddVideo} className="space-y-4">
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder={translate.videoUrlPlaceholder}
                    className="w-full pl-10 pr-4 py-3 bg-white border border-neutral-220 rounded-xl text-xs font-semibold focus:outline-none focus:border-blue-500 shadow-xs"
                  />
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <input
                    ...
                    placeholder={translate.videoTitlePlaceholder}
                    className="w-full px-4 py-3 bg-white border border-neutral-220 rounded-xl text-xs font-semibold focus:outline-none focus:border-blue-500 shadow-xs"
                  />
                  <input
                    type="text"
                    ...
                    placeholder={translate.videoDuration}
                    className="w-full px-4 py-3 bg-white border border-neutral-220 rounded-xl text-xs font-semibold focus:outline-none focus:border-blue-500 shadow-xs"
                  />
                </div>
                ...
                <div className="space-y-1.5 text-left">
                  <label className="text-[10px] font-bold text-neutral-500 uppercase tracking-wider font-mono">
                    {lang === 'vi' ? 'Cách xử lý video khi tải tệp lên:' : 'On file upload, handle as:'}
                  </label>
                  <select
                    value={replaceTarget}
                    onChange={(e) => setReplaceTarget(e.target.value)}
                    className="w-full px-3 py-2.5 bg-white border border-neutral-220 rounded-xl text-xs font-semibold focus:outline-none focus:border-blue-500 shadow-xs cursor-pointer text-neutral-700 font-sans"
                  >
                    <option value="new">
                      {lang === 'vi' ? '➕ Thêm mới hoàn toàn' : '➕ Add as a new video'}
                    </option>
                    {isNdsv ? (
                      <>
                        <option value="ndsv-v1">
                          {lang === 'vi' ? '🔄 Thay thế: Thước phim 1 (Recap Đêm Chung Kết)' : '🔄 Replace: Clip 1 (Recap Gala Final)'}
                        </option>
                        <option value="ndsv-v2">
                          {lang === 'vi' ? '🔄 Thay thế: Thước phim 2 (Hậu trường thiết kế)' : '🔄 Replace: Clip 2 (Behind the Scenes)'}
                        </option>
                      </>
                    ) : (
                      <>
                        <option value="emis-v1">
                          {lang === 'vi' ? '🔄 Thay thế: Thước phim 1 (Wind Garden - Emi Cafe)' : '🔄 Replace: Clip 1 (Wind Garden)'}
                        </option>
                        <option value="emis-v2">
                          {lang === 'vi' ? '🔄 Thay thế: Thước phim 2 (An Thượng - Emi Cafe)' : '🔄 Replace: Clip 2 (An Thuong Area)'}
                        </option>
                        <option value="emis-v3">
                          {lang === 'vi' ? '🔄 Thay thế: Thước phim 3 (Phinn Gốm Cafe)' : '🔄 Replace: Clip 3 (Phinn Gốm Cafe)'}
                        </option>
                        <option value="emis-v4">
                          {lang === 'vi' ? '🔄 Thay thế: Thước phim 4 (Namto House)' : '🔄 Replace: Clip 4 (Namto House)'}
                        </option>
                      </>
                    )}
                  </select>
                ...
                <button
                  ...
                  className="w-full py-3 rounded-xl bg-purple-600 hover:bg-purple-500 active:scale-99 duration-200 text-white font-bold text-xs cursor-pointer shadow-sm disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-1.5"
                >
                  <Plus className="w-4 h-4" />
                  {translate.uploadBtn}
                </button>
        ...
        {/* Beautiful user guide on integration (Hướng dẫn gửi sản phẩm để hiển thị lên dự án) */}
        <motion.div 
          initial={{ opacity: 0, y: 15 }}
          animate={{ opacity: 1, y: 0 }}
          className="bg-slate-900 text-white rounded-3xl border border-slate-800 p-8 sm:p-10 shadow-xl text-left relative overflow-hidden"
        >
          <div className="absolute top-0 right-0 w-48 h-48 bg-blue-500/10 rounded-bl-full pointer-events-none" />
          
          <div className="relative z-10">
            <h3 className="text-xl sm:text-2xl font-extrabold text-white tracking-tight font-sans flex items-center gap-3">
              <span className="w-8 h-8 rounded-lg bg-blue-600 text-white flex items-center justify-center font-bold text-sm">💡</span>
              {lang === 'vi' ? 'HƯỚNG DẪN: Làm thế nào để gửi Video thực tế và ghim lên dự án của bạn?' : 'GUIDE: How to submit actual videos and display them on this app?'}
            </h3>
            <p className="text-slate-400 text-xs sm:text-sm mt-2 leading-relaxed max-w-4xl font-light">
              {lang === 'vi' 
                ? 'Để đảm bảo thành phẩm video thực tế của bạn hiển thị và lưu giữ vĩnh viễn trên trang web này, bạn có hai phương thức cực kỳ tiện lợi dưới đây:' 
                : 'To ensure your real video production assets are displayed permanently and stored correctly, you can use either of these two methods:'}
            </p>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
              {/* Method 1 Card */}
              <div className="bg-slate-800/60 border border-slate-700/50 p-6 rounded-2xl flex flex-col justify-between">
                <div>
                  <span className="text-[10px] font-bold text-blue-400 font-mono uppercase tracking-wide bg-blue-500/10 px-2.5 py-1 rounded-full">{lang === 'vi' ? 'Cách 1: Thêm nhanh qua giao diện' : 'Method 1: Interactive Client UI'}</span>
                  <h4 className="text-sm font-bold text-white mt-3 font-sans">
                    {lang === 'vi' ? 'Sử dụng bộ tải lên lưu trữ cục bộ (LocalStorage)' : 'Instant Local Browser Session Persistence'}
                  </h4>
                  <ul className="text-[11.5px] text-slate-400 leading-relaxed font-light mt-3 space-y-2 list-disc pl-4">
                    <li>{lang === 'vi' ? 'Sử dụng trực tiếp ô tải video/ảnh bên trên' : 'Directly use the upload forms on this page.'}</li>
                    <li>{lang === 'vi' ? 'Hỗ trợ kéo thả xem trước nhanh, hoặc dán link video trực tiếp (.mp4 URL, Youtube link, hoặc mã nhúng TikTok)' : 'Paste active URLs (.mp4 links, YouTube shares, or TikTok embeds).'}</li>
                    <li><strong>{lang === 'vi' ? 'Ưu điểm:' : 'Benefit:'}</strong> {lang === 'vi' ? 'Tự động sao lưu vào LocalStorage của trình duyệt, tải lại trang hay đổi ngôn ngữ vẫn không bị mất nội dung!' : 'Automatically backs up to your browser LocalStorage, persistent even across page reloads!'}</li>
                  </ul>
                </div>
              </div>
              ...
              <div className="bg-slate-800/60 border border-slate-700/50 p-6 rounded-2xl flex flex-col justify-between">
                <div>
                  <span className="text-[10px] font-bold text-purple-400 font-mono uppercase tracking-wide bg-purple-500/10 px-2.5 py-1 rounded-full">{lang === 'vi' ? 'Cách 2: Đồng bộ vào mã nguồn' : 'Method 2: Commit into Source Code'}</span>
                  <h4 className="text-sm font-bold text-white mt-3 font-sans">
                    {lang === 'vi' ? 'Kéo thả trực tiếp vào AI Studio & Nhờ trợ lý tích hợp' : 'Upload file to Workspace and ask me to link'}
                  </h4>
                  <ul className="text-[11.5px] text-slate-400 leading-relaxed font-light mt-3 space-y-2 list-disc pl-4">
                    <li>{lang === 'vi' ? 'Kéo thả file video (.mp4) của bạn vào mục lục tệp tin ở thanh biên trái cửa sổ AI Studio' : 'Drag and drop your MP4 files directly into the file explorer on the left of AI Studio workspace.'}</li>
                    <li>{lang === 'vi' ? 'Nhắn tin trò chuyện với tôi: "Tôi vừa tải video lên, hãy ghim vĩnh viễn tệp này vào mã nguồn"' : 'Send a chat message saying: "I have uploaded the video files, please hard-code them into the layout components for permanent display".'}</li>
                    <li><strong>{lang === 'vi' ? 'Ưu điểm:' : 'Benefit:'}</strong> {lang === 'vi' ? 'Hiển thị mặc định cho mọi người dùng truy cập ở bất kỳ đâu trên thế giới sau khi xuất bản!' : 'Permanently bundled in the source code; visible to anyone on the web globally upon deployment!'}</li>
                  </ul>
                </div>
              </div>
            </div>
